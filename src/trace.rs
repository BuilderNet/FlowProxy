...
/// Environment variable for Open Telemetry filter configuration.
...
pub fn init_tracing() -> SdkTracerProvider {
    ...
    let otel_filter =
        if std::env::var(OTEL_FILTER).as_ref().map(String::len).unwrap_or_default() > 0 {
    ...
    // let otel_layer = tracing_opentelemetry::layer().with_tracer(tracer).with_filter(otel_filter);
    // if log_json {
    ...
    // TODO: distinguish json vs plain text logging via param
    let fmt_layer = fmt::layer().json().with_filter(env_filter);
